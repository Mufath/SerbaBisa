...
def pdf_to_word():
    if not HAS_PDF2DOCX:
        return jsonify(error="Aduh, package pdf2docx-nya belum ada nih. Install dulu ya: pip install pdf2docx"), 400
...
def ocr_pdf():
    ...
    try:
        ...
        return jsonify(error="Aplikasi Tesseract nggak ketemu nih. Install dulu dari webnya (https://github.com/tesseract-ocr/tesseract) dan pastiin masuk PATH ya."), 400
    except Exception as e:
        msg = str(e)
        if "language" in msg.lower() or "traineddata" in msg.lower():
            return jsonify(error=f"Bahasa '{lang}' belum di-install. Download dulu file .traineddata-nya dan masukin ke folder tessdata Tesseract kamu ya."), 400
        return jsonify(error=f"Gagal baca teksnya nih (OCR Error): {msg}"), 400
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        if ext == "dwg":
            if not ODA_CONVERTER:
                return jsonify(error="Wah, khusus buat file DWG, kamu butuh aplikasi ODA File Converter gratis terinstal di komputermu (cek opendesign.com/guestfiles). Atau akalin aja dengan ubah DWG ke DXF dulu sebelum masukin ke sini."), 400
