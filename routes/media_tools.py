...
def convert_audio():
    if request.method == "GET":
        return render_template(
            ...
            description="Ubah lagu/suara ke format lain (MP3, WAV, dll)",
...
def convert_video():
    if request.method == "GET":
        return render_template(
            ...
            description="Ubah video ke format lain (MP4, MKV, dll)",
            notes=_ffmpeg_available_notes(),
...
def extract_audio():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Ekstrak Suara",
            description="Ambil suara/lagu aja dari sebuah video",
            notes=_ffmpeg_available_notes(),
...
def trim():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Potong Media",
            description="Potong bagian awal/akhir video atau audio",
            notes=_ffmpeg_available_notes(),
            endpoint="/media/trim",
...
def compress_video():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Kompres Video",
            description="Kecilin ukuran MB video-mu",
            notes=_ffmpeg_available_notes(),
...
def video_to_gif():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Video ke GIF",
            description="Bikin meme GIF dari video",
            notes=_ffmpeg_available_notes()
...
def subtitle_convert():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Konversi Subtitle",
            description="Ubah format subtitle SRT ke VTT atau sebaliknya",
...
def burn_subtitles():
    if request.method == "GET":
        return render_template(
            "upload_tool.html",
            title="Tanam Subtitle",
            description="Patenkan subtitle langsung ke dalam video",
            notes=_ffmpeg_available_notes()
